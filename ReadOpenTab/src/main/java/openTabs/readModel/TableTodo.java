...
public class TableTodo {
	...
	public TableTodo(long tabId, int tableNumber, String waiter, List<ItemTodo> drinksToServe,
			List<ItemTodo> foodToServe, List<ItemTodo> inPreparation) {
		super();
		this.tabId = tabId;
	...
	public TableTodo(long tabId, int tableNumber, String waiter) {
		this(tabId, tableNumber, waiter, new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
	}
	...
	public ItemTodo removeByMenuNumberDrinkToServe(Integer num) {
		ItemTodo item = drinksToServe.stream().filter(i -> i.getMenuNumber() == num).findFirst().get();
		drinksToServe.remove(item);
		return item;
	}

	public ItemTodo removeByMenuNumberFoodToServe(Integer num) {
		ItemTodo item = foodToServe.stream().filter(i -> i.getMenuNumber() == num).findFirst().get();
		foodToServe.remove(item);
		return item;
	}

	public ItemTodo removeByMenuNumberInPreparation(Integer num) {
		ItemTodo item = inPreparation.stream().filter(i -> i.getMenuNumber() == num).findFirst().get();
